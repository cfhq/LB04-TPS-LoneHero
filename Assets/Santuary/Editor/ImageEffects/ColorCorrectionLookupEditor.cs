            ...
            if (tex && (target as ColorCorrectionLookup).basedOnTempTex != AssetDatabase.GetAssetPath(tex))
            {
                ...
                else if (GUILayout.Button ("Convert and Apply"))
                {
                    ...
                    if (string.IsNullOrEmpty(path)) {
                        Debug.LogWarning("[ColorCorrectionLookupEditor] Couldn't get asset path for selected texture.");
                    } else {
                        TextureImporter textureImporter = AssetImporter.GetAtPath(path) as TextureImporter;
                        if (textureImporter == null) {
                            Debug.LogWarningFormat("[ColorCorrectionLookupEditor] AssetImporter at path '{0}' is not a TextureImporter.", path);
                        } else {
